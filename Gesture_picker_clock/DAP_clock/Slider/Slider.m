...
- (void)drawRect:(CGRect)rect
{
    ...
    NSLog(@"the angle:%f",3*M_PI/2-ToRad(angle+3));
    ...
    if(labelsEvenSpacing != nil&&isShowTimeLabel) {
        [self drawLabels:ctx];
    }
...
-(void) drawHandle:(CGContextRef)ctx{
    ...
    if(_handleType == semiTransparentWhiteCircle) {
        ...
        CGContextAddArc(ctx, self.frame.size.width/2  , self.frame.size.height/2, radius, 3*M_PI/2-ToRad(angle+40), 3*M_PI/2-ToRad(angle+40)+1.2, 0);
                NSLog(@"the handler location:%f,%f",handleCenter.x,handleCenter.y);
        [_filledColor setStroke];
        CGContextSetLineWidth(ctx, _lineWidth);
        CGContextSetLineCap(ctx, kCGLineCapButt);
        CGContextDrawPath(ctx, kCGPathStroke);
